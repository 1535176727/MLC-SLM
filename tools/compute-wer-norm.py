...
from local.Whisper_normalization import WhisperNormalizer
...
whisper_normalizer = WhisperNormalizer()

def is_vietnamese_char(char):
    try:
        name = unicodedata.name(char)
        return 'LATIN' in name and (
            'WITH' in name or  # 表示变音
            char in ['ă', 'â', 'đ', 'ê', 'ô', 'ơ', 'ư',
                     'Ă', 'Â', 'Đ', 'Ê', 'Ô', 'Ơ', 'Ư']
        )
    except ValueError:
        return False
def characterize(string):
    ...
    while i < len(string):
        ...
        if cat1 == 'Lo':
            ...
            while j < len(string):
                try:
                    if 'CYRILLIC' not in unicodedata.name(string[j]):
                        break
                except:
                    break
                j += 1
            ...
            i = j
        elif is_vietnamese_char(char):  # 新增：处理越南语词
            j = i + 1
            while j < len(string):
                if not is_vietnamese_char(string[j]) and not string[j].isalpha():
                    break
                j += 1
            res.append(string[i:j])
            i = j
...
def normalize(sentence, ignore_words, cs, split=None,key=None):
    ...
    #filtered_sentence = ' '.join(new_sentence)
    #normalized = whisper_normalizer.normalize(filtered_sentence,key=key)
...
if __name__ == '__main__':
    ...
    with codecs.open(hyp_file, 'r', 'utf-8') as fh:
        for line in fh:
            ...
            #                          split)
            rec_set[fid] = normalize(array[1:], ignore_words, case_sensitive, split, key=fid)
    ...
    for line in open(ref_file, 'r', encoding='utf-8'):
        ...
        if fid not in rec_set:
            continue
        lab = normalize(array[1:], ignore_words, case_sensitive, split, key=fid)
        rec = rec_set[fid]
        if verbose:
            print('\nutt: %s' % fid)
