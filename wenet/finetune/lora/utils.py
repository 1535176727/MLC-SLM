...
def inject_lora(module, lora_config):
    ...
    lora_alpha = lora_config["lora_alpha"]
    lora_dropout = lora_config["lora_dropout"]
    lora_list = lora_config["lora_list"]

    # 遍历子模块
    for name, submodule in module.named_children():
        if name in lora_list and isinstance(submodule, torch.nn.Linear):
            if name not in lora_list:
                continue
            if name not in ['linear_q', 'linear_v']:  # 显式限定白名单
                continue
            in_features = submodule.in_features
            out_features = submodule.out_features

            # 创建 lora Linear（用你自己的 lora.Linear 类替换这里）
            ...
            # 拷贝原始权重（可选）
            lora_linear.weight.data.copy_(submodule.weight.data)
            if hasattr(submodule, "bias") and submodule.bias is not None:
                lora_linear.bias = submodule.bias  # 保留原 bias
