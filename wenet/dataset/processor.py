...
def detect_language(sample, limited_langs):
    ...
    language = [sample["key"].split('-')[0]]
    if len(language) == 0:
        raise ValueError("No language found in the key")
...
def speed_perturb(sample, speeds=None):
    ...
    if speed != 1.0:
        ...
        new_freq = int(orig_freq * speed)
        try:
            # Resample to new "fake" speed, then resample回来
            resampler1 = T.Resample(orig_freq, new_freq)
            resampler2 = T.Resample(new_freq, orig_freq)
            wav = resampler2(resampler1(waveform))
            sample['wav'] = wav
        except Exception as e:
            logging.warning(f"Resample speed perturb failed: {e}")
            sample['wav'] = waveform
    else:
        sample['wav'] = waveform
...
#     return sample
def tokenize(sample, tokenizer: BaseTokenizer):
    """Tokenize text with language and task tokens for Whisper

    Args:
        sample: {key, wav, txt, lang, sample_rate, ...}
            - 必须包含 sample['lang'] 语言码（如 'zh', 'en'）

    Returns:
        sample dict with 'label' = input_ids
    """
    tokenizer_name = "qwen"
    if tokenizer_name == "whisper":
        assert 'txt' in sample
        assert 'key' in sample  # 你需要提前保证 sample 中包含 lang 字段
        lang = sample['key'].split('-')[0]  # 假设 key 格式为 'lang-xxx'
        language_mapping={
        "English": "en", "French": "fr", "German": "de", "Italian": "it", "Portuguese": "pt",
        "Spanish": "es", "Japanese": "ja", "Korean": "ko", "Russian": "ru", "Thai": "th", "Vietnamese": "vi"
        }
        lang = language_mapping.get(lang, '').strip()
        # 构造 prefix，例如 <|zh|><|transcribe|><|notimestamps|>
        prefix = f"<|startoftranscript|><|"+lang+ f"|><|transcribe|><|notimestamps|>"
        fix = f"<|endoftext|>"
        # 拼接完整文本
        full_text = prefix + sample['txt']
        # 使用 tokenizer 编码
        label = tokenizer(full_text, add_special_tokens=False)
...
def padding(data):
    ...
    original_wavs = [sample[i]['wav'].squeeze(0) for i in range(len(sample))] 
    ...
    # original_wavs 也先是 list
    original_wavs_list = [sample[i]['wav'].squeeze(0) for i in range(len(sample))]
    batch = {
        "keys": sorted_keys,
        "feats": padded_feats,
