...
def main():
    ...
    if configs.get('tokenizer', None) == 'whisper':
    ...
    # Freeze some modules
    if configs['model'] == 'SHNU-mASR':
        #if configs['freeze_whisper']:
        #    for name, param in model.encoder.named_parameters():
        #        param.requires_grad = False
        #        # logging.info(f"{name} is frozen")
        if "freeze_whisper" in configs and configs["freeze_whisper"]:
            if hasattr(configs,"use_whisper_lora") and configs['use_whisper_lora']:
                for name, param in model.whisper_encoder.named_parameters():
                    if 'lora' not in name:
                        param.requires_grad = False
                    else:
                        param.requires_grad = True
            else:
                for _, param in model.whisper_encoder.named_parameters():
                    param.requires_grad = False
        else:
            for _, param in model.whisper_encoder.named_parameters():
                param.requires_grad = True
        ...
        if hasattr(configs,"use_qwen2audioencoder") and configs['use_qwen2audioencoder']:
            for name, param in model.encoder_qwen_audio.named_parameters():
                param.requires_grad = False
                # logging.info(f"{name} is frozen")
        if "freeze_mhubert" in configs:
            if configs['freeze_mhubert']:
                for _, param in model.hubert_encoder.named_parameters():
                    param.requires_grad = False
            else:
                for name, param in model.hubert_encoder.named_parameters():
                    if 'feature_extractor' in name or 'masked_spec_embed' in name:
                        param.requires_grad = False
                    else:
                        param.requires_grad = True
        # if configs['freeze_projector']:
        #     for name, param in model.encoder_projector.named_parameters():
        #         param.requires_grad = False
        # 修复后的正确代码
        if configs.get('freeze_projector', False):
            logging.info("Attempting to freeze projector modules...")
            
            # 方案一：检查并冻结线性Projector (如果存在)
            if hasattr(model, 'encoder_projector'):
                logging.info("Found and freezing 'encoder_projector'.")
                for param in model.encoder_projector.parameters():
                    param.requires_grad = False
            
            # 方案二：检查并冻结Q-Former相关的模块 (如果它们存在)
            if hasattr(model, 'speech_former'):
                logging.info("Found and freezing 'speech_former'.")
                for param in model.speech_former.parameters():
                    param.requires_grad = False
            
            if hasattr(model, 'speech_query_tokens'):
                logging.info("Found and freezing 'speech_query_tokens'.")
                # 注意：Parameter对象直接设置requires_grad即可，它没有.parameters()方法
                model.speech_query_tokens.requires_grad = False
                
            if hasattr(model, 'final_projector'):
                logging.info("Found and freezing 'final_projector'.")
                for param in model.final_projector.parameters():
                    param.requires_grad = False
            if hasattr(model, "bicross_attn"):
                logging.info("Found and freezing 'cross_attn'.")
                for param in model.bicross_attn.parameters():
                    param.requires_grad = False
        else:
            if hasattr(model, "bicross_attn"):
                for param in model.bicross_attn.parameters():
