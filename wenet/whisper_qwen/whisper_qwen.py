...
class WhisperQwen(torch.nn.Module):
    ...
    def __init__(
        self,
        encoder: BaseEncoder,
        qwen2audioencoder: torch.nn.Module,
        use_qwen2audioencoder: bool,
        qwen: AutoModelForCausalLM,
        tokenizer: AutoTokenizer,
        use_qwen_lora: bool,
        use_mhubert: bool,
        hubert_encoder: torch.nn.Module,
        ...
        self.qwen2audioencoder = qwen2audioencoder
        self.use_qwen2audioencoder = use_qwen2audioencoder
        self.use_qwen_lora = use_qwen_lora
        self.use_mhubert = use_mhubert
        self.hubert_encoder = hubert_encoder
        self.hubert_processor = hubert_processor
        # === 这里新增：启用 gradient checkpointing（如果支持） ===
        ...
        if self.qwen2audioencoder is not None and hasattr(self.qwen2audioencoder, "gradient_checkpointing_enable"):
            self.qwen2audioencoder.gradient_checkpointing_enable()
    ...
    def decode(
        ...
        if encoder_out.shape[1] > encoder_out_hubert.shape[1]:
            ...
            encoder_out_hubert = encoder_out_hubert[:, :-1, :]
        assert encoder_out.shape[1] == encoder_out_hubert.shape[1]
        # 2. Concatenate encoder_out and encoder_out_hubert
        encoder_out = torch.cat([encoder_out, encoder_out_hubert], dim=2)

        # 4. WhisperProjector
        speech_embeds = self.encoder_projector(encoder_out)  # (B, T, D')
