...
def init_dataset_and_dataloader(args, configs, tokenizer, seed=777):
    ...
    if configs['tokenizer'] == 'qwen':
        ...
        configs['vocab_size'] = tokenizer.vocab_size
        print(
            f"vocab_size: {configs['vocab_size']}, "
            f"tokenizer_type: {configs['tokenizer']}")
...
def save_model(model, info_dict):
    ...
    if info_dict["train_engine"] == "deepspeed":
        ...
        with torch.no_grad():
            #print("save checkpoint,line 621 train_utils.py ******************************")
            #print(model)
       ...
       #print("save checkpoint,line 637 train_utils.py ******************************")
       #print(model)
    # save yaml
    if rank == 0:
...
def update_parameter_and_lr(model, optimizer, scheduler, scaler, info_dict):
    ...
    accum_grad = info_dict.get('accum_grad', 1)
    use_amp = info_dict.get('use_amp', False)
    clip = info_dict.get('grad_clip', 5.0)
