...
#8393M
    def __init__(
        ...
        if self.cross_type == "cross_attn":
            ...
            self.encoder_projector = WhisperProjector(downsample_rate=4, idim=2560, odim=3584)
        else:
            self.encoder_projector = WhisperProjector(downsample_rate=4, idim=2048, odim=896)
        ...
        self.lang_prompts = {
            "English": "Please transcribe the following audio in English:\n",
            "French": "Veuillez transcrire l'audio suivant en français :\n",
            "German": "Bitte transkribieren Sie die folgende deutsche Audioaufnahme:\n",
            "Italian": "Si prega di trascrivere il seguente audio in italiano:\n",
            "Japanese": "以下の日本語音声を書き起こしてください：\n",
            "Korean": "다음 한국어 오디오를 텍스트로 변환해 주세요:\n",
            "Portuguese": "Por favor, transcreva o áudio a seguir em português:\n",
            "Russian": "Пожалуйста, расшифруйте следующую аудиозапись на русском языке:\n",
            "Spanish": "Por favor, transcriba el siguiente audio en español:\n",
            "Thai": "โปรดถอดเสียงไฟล์เสียงต่อไปนี้เป็นภาษาไทย:\n",
            "Vietnamese": "Vui lòng chuyển mã âm thanh sau sang văn bản bằng tiếng Việt:\n",
            # 添加一个默认值，用于处理未在字典中定义的语言
            "default": "Please transcribe the following audio:\n"
        }
    ...
    def prompt_wrap_with_languages(self, speech_embeds, languages):
        ...
        prompt_texts = [
            self.lang_prompts.get(lang, self.lang_prompts["default"]) for lang in languages
        ]
