...
export NCCL_LAUNCH_TIMEOUT=6000    # 默认 600 秒
...
if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
  ...
  python wenet/bin/recognize_SHNU-mASR.py --gpu 0 \
    --modes $decode_mode \
    --config $step3_dir/train.yaml \
    --data_type $data_type \
    --test_data $test_data_dir/${data_type}_data.list \
    --checkpoint $step3_dir/epoch_0.pt \
    --batch_size 500 \
    --dtype bf16 \
...
if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
  for lang in English-American English-Australian English-British English-Filipino English-Indian French German Italian Japanese Korean Portuguese Russian Spanish Thai Vietnamese; do
    grep $lang $step3_dir/$decode_mode/text > $step3_dir/$decode_mode/text_$lang
    python tools/compute-wer-norm.py --char=1 --v=1 \
        $test_data_dir/text_tn $step3_dir/$decode_mode/text_$lang > $step3_dir/$decode_mode/wer_$lang
  done
  python tools/compute-wer-norm.py --char=1 --v=1 \
        $test_data_dir/text_tn $step3_dir/$decode_mode/text > $step3_dir/$decode_mode/wer
fi
